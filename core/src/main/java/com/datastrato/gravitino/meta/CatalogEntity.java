...
public class CatalogEntity implements Entity, Auditable, HasIdentifier {
  ...
  public CatalogInfo toCatalogInfo() {
    return new CatalogInfo(entityMetadata.getId(), entityMetadata.getName(), type, provider, entityMetadata.getComment(), entityMetadata.getProperties(), auditInfo, namespace);
  }

  public CatalogInfo toCatalogInfoWithoutHiddenProps(Set<String> hiddenKeys) {
    Map<String, String> filteredProperties =
        entityMetadata.getProperties() == null ? new HashMap<>() : new HashMap<>(entityMetadata.getProperties());
    filteredProperties.keySet().removeAll(hiddenKeys);
    return new CatalogInfo(
            entityMetadata.getId(), entityMetadata.getName(), type, provider, entityMetadata.getComment(), filteredProperties, auditInfo, namespace);
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hashCode(entityMetadata.getId(), entityMetadata.getName(), type, provider, entityMetadata.getComment(), entityMetadata.getProperties(), auditInfo);
  }
